...
export default function ResolvedReportsPage() {
  ...
  const fetchResolved = async () => {
    try {
      const res = await api.get('/reports', { params: { resolved_only: 'true' } });
      setReports(res.data);
    } catch (err) {
      console.error('Could not load resolved reports:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      ...
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-text-main">Resolved Reports</h1>
